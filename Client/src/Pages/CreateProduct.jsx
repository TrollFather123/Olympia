import { yupResolver } from '@hookform/resolvers/yup';
import React, { useState } from 'react'
import { useForm } from 'react-hook-form';
import { useDispatch } from 'react-redux';
import { useNavigate } from 'react-router-dom';
import { ProductSchema, UserSchema } from '../Validation/Login.schema';
import DashBoardWrapper from '../Layout/DashBoardWrapper';
import styled from '@emotion/styled';
import { Box, Button, Container, Grid, TextField } from '@mui/material';
import ErrorText from '../Validation/ErrorText';
import { CreateEachProduct } from '../Redux/ProductSlice';


const FormWrapper = styled(Box)`
`;


export default function CreateProduct() {
    const dispatch = useDispatch()
    const navigate = useNavigate()
    const [images, setImages] = useState([])
    const {
      handleSubmit,
      register,
      setValue,
      formState: { errors },
      reset,
    } = useForm({
      resolver: yupResolver(ProductSchema),
    });


    const FormSubmit = (product) =>{
       
        dispatch(CreateEachProduct({...product,images:images})).unwrap().then((res)=>{res && alert("Product Added Successfully");navigate("/dashboard/products")}).catch((err)=>alert(err))
        reset()
    }


  return (
   <DashBoardWrapper>
       <FormWrapper>
        <Container fixed>
        <Grid container spacing={3}>
          <Grid item xs={12}>
            <Box className="form_group">
              <TextField label="" variant="outlined" placeholder="Title" fullWidth {...register("title")}/>
              <ErrorText text={errors?.title?.message}/>
            </Box>
          </Grid>
          <Grid item xs={12}>
            <Box className="form_group">
              <TextField
                label=""
                variant="outlined"
                placeholder="price"
          
                fullWidth
                {...register("price")}
              />
              <ErrorText text={errors?.price?.message}/>
            </Box>
          </Grid>
          <Grid item xs={12}>
            <Box className="form_group">
              <TextField
                label=""
                variant="outlined"
                placeholder="description"
          
                fullWidth
                {...register("description")}
              />
              <ErrorText text={errors?.description?.message}/>
            </Box>
          </Grid>
          <Grid item xs={12}>
            <Box className="form_group">
              <TextField
                label=""
                variant="outlined"
                placeholder="categoryId"
          
                fullWidth
                {...register("categoryId")}
              />
              <ErrorText text={errors?.categoryId?.message}/>
            </Box>
          </Grid>
       
          <Grid item xs={12}>
            <Box className="form_group">
              <input type="file" onChange={(e)=>setImages("images",e.target.files[0])}/>
       
            </Box>
          </Grid>
          <Grid item xs={12}>
            <Button variant="contained" onClick={handleSubmit(FormSubmit)}>
                Submit
            </Button>
            </Grid>
        </Grid>
        </Container>
      </FormWrapper>
   </DashBoardWrapper>
  )
}
